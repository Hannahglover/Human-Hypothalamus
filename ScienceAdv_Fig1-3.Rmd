...
Neurons_Reassigned = subset(KaZhouAll_Unassigned_3D, KaZhouAll_Unassigned_3D$umap_1 0.5 & KaZhouAll_Unassigned_3D$umap_2 > -2 & KaZhouAll_Unassigned_3D$umap_3 < 2)
...
Tanycytes_Reassigned = subset(KaZhouAll_Unassigned_3D, ! KaZhouAll_Unassigned_3D$X %in% Astrocytes_Reassigned$X & KaZhouAll_Unassigned_3D$umap_1 > -2 &  KaZhouAll_Unassigned_3D$umap_1 < 2.5 & KaZhouAll_Unassigned_3D$umap_2 0.5 & KaZhouAll_Unassigned_3D$umap_3 > 3 & KaZhouAll_Unassigned_3D$umap_3 < 8)
...
OligCheck2 = subset(KaZhouAll_UMAP, row.names(KaZhouAll_UMAP) %in% colnames(OLIG_Seu) & KaZhouAll_UMAP$UMAP_1 0.5 & KaZhouAll_UMAP$UMAP_2 < 0)
...
OligCheck4 = subset(KaZhouAll_UMAP, row.names(KaZhouAll_UMAP) %in% colnames(OLIG_Seu) & KaZhouAll_UMAP$UMAP_1 0.5 & KaZhouAll_UMAP$UMAP_2 0.5 & KaZhouAll_UMAP$UMAP_2 < 8)
...
ClusteForIntProg_Pt1 = subset()
CleanedClusters_Figure1 = read.csv("~/Dropbox/LabMac/CleanedClusters_Figure1_19DEC22.csv", row.names =2)

ClusteForIntProg2 = subset(KaZhouAll@meta.data, KaZhouAll@meta.data$sample %in% c("CS13", "CS14", "CS15",  "CS22_2_hypo", "CS22_hypo", "GW7_Lane1", "GW7_Lane2", "GW8_1", "GW8_2") & KaZhouAll@meta.data$Fig1Clusters == "Neuronal" | KaZhouAll@meta.data$Fig1Clusters == "Neuroepithelial")
ClusteForIntProg2_Seu = subset(KaZhouAll, cells = row.names(ClusteForIntProg2))
Rest_Seu = subset(KaZhouAll, cells = row.names(ClusteForIntProg2), invert=T)

FP = FeaturePlot(KaZhouAll, c("TOP2A", "CDK1", "ALDOC", "ALDH1L1", "GPR50", "TBX3", "COL25A1", "COL23A1", "ELAVL4", "DCX", "PDGFRA", "SOX10", "OLIG1", "DEUP1", "FOXJ1", "HOPX", "VCAM1", "NKX6-1", "FGF8"), raster = T)
pdf(paste(FilePath, "Fig1Supp_RG_All.pdf", sep=""), width = 20, height = 20)
print(FP)
dev.off()

FP = FeaturePlot(KaZhouAll, unique(c("PCLAF", "ALDOC", "PLA2G7", "CLU", "EDNRB", "AW047730", "NDNF", "NID1", "DKK3", "", "KCNE1L", "GDF10", "FOXG1", "FOXJ1", "TM4SF1", "WNT8B", "DYNLRB2", "MEIG1", "SHISA8", "IFITM3", "DEUP1", "RARRES2", "CCNO", "MYB", "NTN1", "WLS", "RIIAD1", "RSPH1", "CALML4", "CFAP126", "GM28050", "PLTP", "CLEC3B", "FAM183B", "CDKN1A", "PIFO", "CHCHD10", "MLF1", "HIPK1", "TMEM107", "RNASE4", "TEKT1", "ODF3B", "DMKN", "HSPA2", "EZR", "RSPH4A", "TPPP3", "GM19935", "CLU", "ENKUR", "GAS6", "ACOT5", "CCDC39", "CFAP45", "HDC", "RSPO2", "LRRC23", "TUBB3", "IGFBPL1", "ELAVL4", "INSM1", "NHLH2", "ELAVL3", "TAGLN3", "INA", "GADD45G", "ABRACL", "ELAVL2", "DCX", "HES6", "MLLT11", "PDGFRA", "CNTN1", "OLIG1", "SERPINE2", "GPR17", "LHFPL3", "CSPG4", "C1QL1", "SOX10", "OLIG2", "NCALD", "PCDH11X", "MFSD2A", "PCDH15", "SCRG1", "EPN2", "BCAN", "GRIA2", "OPCML", "PID1", "CACNG4", "CDO1", "ENPP2", "ANKS1B", "GPR37L1", "GM45153", "CAR8", "MATN4", "PLPPR1", "OLFM2", "PTPRE", "QPCT", "ZDHHC2", "GM2A", "PHLDA1", "MMP15", "BRINP3", "RGCC", "KCND2", "GM42583", "DNM3", "OSTF1", "TPM1", "SAPCD2", "PIK3R1", "ALCAM", "SPRY1", "MMP16", "GATM", "SHH", "NKX2-4", "RAX", "SLIT2", "NKX2-4", "CCND2", "COL25A1", "", "ATF3", "CDKN1C", "", "DLK1", "RAX", "SIX3", "COL23A1", "FOSB", "GPC3", "TBX3", "SCN9A", "CEBPD", "MIA", "SIX3OS1", "FBN2", "IGFBP2", "SIX6", "PRELP", "FZD5", "MAF", "NKX2-4", "SPARCL1", "CLU", "S100A6", "SLC7A11", "CLDN10", "NTRK2", "SERPINE2", "DTNA", "NKAIN4", "ATP1B1", "SLC6A11", "HOPX", "PDZPH1", "RAMP1", "PBXIP1", "PREX2", "MIA", "SIX6", "IGFBPL1", "CP", "SIX3OS1", "H19", "FOSB", "MAF", "ZIC1", "IGFBP5", "COL2A1", "SELENOP")), ncol=8, raster = T)
pdf(paste(FilePath, "Fig1Supp_RG_All.pdf", sep="") , width = 50, height = 50)
print(FP)
dev.off()


FP = FeaturePlot(KaZhouAll, unique(c("SOX2", "VIM", "NES", "HES1", "MKI67", "FABP7", "SLC1A3", "TTYH1", "HMGA2", "FAM107A", "NKX2-1", "ASCL1", "NHLH2", "DLX1", "DLX2", "HMGA2", "ARHGAP28", "TTYH1", "FAM107A", "VIM1", "NES", "SOX2", "PAX6", "TOP2A", "CDK1", "PDGFRA", "SOX10", "OL1G1", "DEUP1", "FOXJ1", "HOPX", "VCAM1", "NKX6-1", "FGF8", "SIX6", "IGFBPL1", "CP", "SIX3OS1", "H19", "FOSB", "MAF", "ZIC1", "IGFBP5", "COL2A1", "SELENOP")), raster = T)
pdf(paste(FilePath, "Fig1Supp_RG_Zhang.pdf", sep="") , width = 15, height = 20)
print(FP)
dev.off()

FP = FeaturePlot(KaZhouAll, unique(c("MAFB", "PDZPH1", "FRZB", "SCN7A", "FOXJ1", "RAX", "ADM",  "EMX2", "LHX2", "TMEM212", "CRYM", "CCDC153", "RFX2", "RFX3", "RFX4", "NPAS3", "COL23A1", "CD59", "SLC17A8", "VCAN", "FRZB", "PENK", "COL25A1", "CACNA2D2", "STOML3", "TMEM212", "SLC7A11", "PCP4L1", "PLTP", "TGFB2", "NR2E1", "EPHB1", "P3H2", "NELL2", "FGF10", "RGCC", "GRIA2", "RLBP1", "SIX6", "SCN7A", "MEST", "IGFBP5", "RGS7BP")), raster = T)
pdf(paste(FilePath, "Fig1Supp_RG_Tany.pdf", sep="") , width = 20, height = 30)
print(FP)
dev.off()

FP = FeaturePlot(ClusteForIntProg2_Seu, c("STMN2", "SYT1", "VIM1", "NEUROG2", "ASCL1", "EBF2", "NHLH1", "EBF3", "RND2", "RSPO3", "OTP", "SOX2", "LHX2", "VIM", "HES1", "NGN2", "SOX11", "DLL1", "DLL2", "DLX1", "DLX2"), raster = T)
pdf(paste(FilePath, "Fig1Supp_IntProg_Reclusterable.pdf", sep=""), width = 20, height = 20)
print(FP)
dev.off()

FP = FeaturePlot(Rest_Seu, c("STMN2", "SYT1", "VIM1", "NEUROG2", "ASCL1", "EBF2", "NHLH1", "EBF3", "RND2", "RSPO3", "OTP", "SOX2", "LHX2", "VIM", "HES1", "NGN2", "SOX11", "DLL1", "DLL2", "DLX1", "DLX2"), raster = T)
pdf(paste(FilePath, "Fig1Supp_IntProg_AlreadyAssigned.pdf", sep=""), width = 20, height = 20)
print(FP)
dev.off()

DimPlot(KaZhouAll, raster=T)
...
write.csv(CleanedClusters_Figure1, "CleanedClusters_Figure1_15JUL23.csv")
...
#RADIAL GLIA
```{r}
# Ascl1+ IPCs that co-express Sox3, Gsx1 and Isl1 become either glutamatergic or GABAergic neurons. In a similar way, expression of Ngn2 together with Nhlh2, Neurod1 and Otp selectively specifies glutamatergic neurons in the lateral POA and the PVN50 (see below).

#Transcriptional phenotyping of the human hypothalamus51 at GW10 identified six NPC clusters, all co-expressing Vim, Nes and Ascl1. Although GW10 in humans is the period when hypothalamic neurogenesis peaks, the molecular identifiers detected in human NPCs match those in mouse at E11.5 and E15.5. In addition to Vim, Nes and Ascl1, mouse NPCs40,41,50,51 also express Rax, Ngn2, Lhx2, Lhx1, Nhlh2 and Hes1 (Fig. 2d). This classification was recently refined40 by identifying gene modules for three principal NPC clusters destined to topographically different end points in mouse: a cluster expressing Ascl1 and Ngn2 for mediobasal areas, a cluster expressing Lhx1 for anterior territories, and a cluster expressing Ngn2 and Neurod1 for the mammillary complex. Further work in chick embryos associated Foxa2 expression with mammillary and PVN neurogenesis and Six6, Fgf10 and Rax expression with terminal (midventral) regional specification43. A remarkable conclusion from this work is that Rax+ hypothalamic NPCs transiently express Pax6, which otherwise marks the prethalamus, suggesting that prethalamic and midventral NPCs are temporarily coincident38,43.

pdf(paste(FilePath, "Fig1Supp_BeneTextGenes.pdf", sep=""), width = 15, height = 15)
print(FeaturePlot(KaZhouAll, unique(toupper(c("Ascl1", "Sox3", "Gsx1",  "Isl1", "NEUROG2",  "Nhlh2", "Neurod1", "Otp", "Vim", "Nes", "Rax", "Ngn2", "Lhx2", "Lhx1", "Nhlh2", "Hes1", "Foxa2", "Six6", "Fgf10", "Rax", "Pax6"))), raster = T))
dev.off()

#Upper level depicts NPCs (hypothalamic radial glia; hRG) residing in the ventricular zone (VZ) at the wall of the third ventricle (3V). hRGs produce IPCs (hypothalamic mantle radial glia; hmRG) by asymmetric division, which reside in the mantle zone (MZ). Both hRGs and hmRGs radiate processes towards the pial surface.


pdf(paste(FilePath, "Fig1Supp_BeneGenesFigure.pdf", sep=""), width = 15, height = 15)
print(FeaturePlot(KaZhouAll, c("RAX","ASCL1",  "SOX2", "LHX2", "VIM","NES", "HES1", "NEUROG2", "SOX11", "DLL1", "DLL2", "DLX1", "DLX2", "COL23A1"), raster = T))
dev.off()


pdf(paste(FilePath, "Fig1Supp_ZhouGenesFigure.pdf", sep=""), width = 15, height = 15)
print(FeaturePlot(KaZhouAll, c("SOX2", "SOXA1","VIM",  "NES", "HES1", "MKI67","FABP7", "SLC1A3", "TTYH1", "HMGA2", "FAM107A", "NKX2-1", "ASCL1", "NHLH2", "DLX1", "DLX2", "NHLH2", "DLX1", "DLC5", "OLIG1", "OLIG5", "VCAM1", "RAX", "SHH"), raster = T))
dev.off()
```
...
#CleanedClusters_Figure1 = read.csv("~/Dropbox/LabMac/CleanedClusters_Figure1_15JUL23.csv", row.names =2)
...
p = plot_ly(CleanedClusters_Figure1_Meta, x = ~UMAP_3, y = ~UMAP_2, z = ~UMAP_1, sizes = 1, size = 1,color = ~Timepoint2, colors = TimepointColors_FetalZhou)
...
###############################################
#By Assignment
p = plot_ly(CleanedClusters_Figure1_Meta, x = ~UMAP_3, y = ~UMAP_2, z = ~UMAP_1, size = 1, sizes = 1, color = ~Fig1Assignments, colors = CellPopColors, type="scatter3d")
...
MetaTable = as.data.frame(table(Subs$Fig1Assignments))
MetaTable$Percent = MetaTable$Freq/dim(Subs)[1]*100
MetaTable$Freq = NULL
...
################## DOT PLOT ###################
###############################################
KaZhouAll@meta.data$Fig1Assignments = factor(KaZhouAll@meta.data$Fig1Assignments, levels = rev(PopOrder))
...
pdf(paste(FilePath, "Fetal_ForPaper_Fig1DotPlot_JULY23.pdf", sep=""), width = 15, height = 5)
print(DP)
dev.off()
...
CheckGW7 = subset(KaZhouAll@meta.data, KaZhouAll@meta.data$Timepoint2 == "GW8 [Zhou]")

table(CheckGW7$Fig1Assignments)

#GW10: Zhou = 79.7, Kreg = 56.2
```


#FIG1-PLOTS
```{r FIG1-PLOTS}
#FilePath = "~/Library/CloudStorage/Box-Box/HG2553 Main Folder/Science Advances/"
KaZhouAll = readRDS("~/Dropbox/LabMac/KaZhouAll_mt10_integrated.rds")
CleanedClusters_Figure1 = read.csv("~/Dropbox/LabMac/CleanedClusters_Figure1_19DEC22.csv", row.names =2)
...
Idents(KaZhouAll) = "Fig1Assignments"
DefaultAssay(KaZhouAll) = "RNA"
DP = Seurat::DotPlot(KaZhouAll, assay = "RNA", features= c("HMGA2", "ARHGAP28", "MKI67","NES", "FABP7", "TTYH1", "SLC1A3",  "ASCL1", "EGFR", "OLIG1","OLIG2","APOD","PDGFRA",  "MBP", "PLP1", "MAG", "MOG", "STMN2", "SYT1", "GJA1", "GFAP", "AQP4", "ALDOC", "SLC1A2",  "CCDC153",  "CRYM", "C1QB", "AIF1", "RGS5",  "KCNJ8", "ABCC9", "ACTA2", "TAGLN", "LUM", "DCN", "COL1A1",  "PDGFRB", "CLDN5", "ITM2A", "FLT1", "HBA2"),  cols = c("#FFFFFF", HeatmapBlue)) + theme(axis.text.x = element_text(size=10, face="italic", angle = 90, vjust = 0.5, hjust=1), axis.text.y = element_text(size=10), legend.title = element_text(size=8)) + ylab("") #, dot.min = 0.15
...
```{r F2:TRAJECT}
EdKaZhouHypoNeurons = readRDS("~/Downloads/Sci Adv/EdKaZhouHypoNeurons_mt10_integrated.rds")
set.seed(100)
...
####### FIG 2B
p = plot_cells_3d(M3SeuNODE, color_cells_by = "SampleAdult",size = 1, sizes = 1, color_palette = TimepointColors_FetalAdult)
...
pdf(file=paste('./MainLineage_WithLabs_', LIN, '.pdf', sep=""))
plot(net.NODE,layout=lrt.NODE,
 vertex.shape="pie",
 vertex.size=3,
 vertex.pie=perSamp2,
 vertex.label.dist=0.8,
 vertex.label.cex=0.6,
 vertex.label.degree=pi, vertex.frame.color="darkgrey", xlim = c(-2, 2), ylim = c(-1, 1))
dev.off()

pdf(file=paste('./MainLineage_NoLabs_', LIN, '.pdf', sep=""), width = 20, height = 20)
plot(net.NODE,layout=lrt.NODE,
 vertex.shape="pie",
 vertex.size=2.5,
 vertex.pie=perSamp2,
 #edge.width=seq(1,10),
 vertex.label=NA, pie.density = 0.05, vertex.frame.color =list(c("#fecf29","#538d22","#ff7aa2")))
dev.off()
  
}


MRTreeRes = readRDS("~/Downloads/Sci Adv/tree80_L15_treeTrim.rds")
...
dir.create(file.path("./", paste(LIN, "MRTreeToTrajectoryK560/", sep="_")))
for(t in sort(unique(Resolutions$K560))){
PullCluster = subset(Resolutions, Resolutions$K560 == t)
EdKaZhouHypoNeurons$Other = "Other"
EdKaZhouHypoNeurons@meta.data[colnames(EdKaZhouHypoNeurons) %in% row.names(PullCluster),'Other'] = 'TargetPop'

for(i in names(perSamp)){
tmp = table(EdKaZhouHypoNeurons$Other[which(EdKaZhouHypoNeurons$Vertex==i)])
tmp2=c(0,0,0)
names(tmp2)=c("TargetPop","Other")
tmpov = intersect(names(tmp),names(tmp2))
tmp2[tmpov] = tmp[tmpov]
tmpPer = round((tmp2/length(which(EdKaZhouHypoNeurons$Vertex==i)))*100,0)
perSamp[[i]] = tmpPer
...
V(net.NODE)$pie.color=list(c("#fecf29","darkslateblue"))
perSamp2=perSamp[V(net.NODE)$name]

pdf(file=paste('./', LIN, '_MRTreeToTrajectoryK560/MainLineage_NoLabs_', LIN, 'Cluster', t, 'fromK55.pdf', sep=""), width = 20, height = 20)
plot(net.NODE,layout=lrt.NODE,
 vertex.shape="pie",
 vertex.size=2.5,
 vertex.pie=perSamp2,
 vertex.label=NA, pie.density = 0.05, vertex.frame.color =list(c("#fecf29","darkslateblue")))
dev.off()
...
MainAssign2 = MainAssign %>% dplyr::select(Nuclei)
...
ResolutionsAllCells = merge(Resolutions, K560_Annots,  by = "K560", all=T)


K560_AnnotsReduced = ResolutionsAllCells %>% dplyr::select("Nuclei_K560", "K560")
row.names(K560_AnnotsReduced) = ResolutionsAllCells$Barcs
colnames(K560_AnnotsReduced) = c("MRTree_Nuclei", "MRTree_Cluster")

MainAssign3 = merge(MainAssign2, K560_AnnotsReduced, by = 0, all = T)

predictions = readRDS("~/Dropbox/Columbia/predictions.rds")
HypoMapPred = predictions$Region_summarized
HypoMapPred2 = HypoMapPred %>% dplyr::select("predicted.id", "prediction.score.max")
HypoMapPred2 = subset(HypoMapPred2, HypoMapPred2$prediction.score.max >= 0.3)
colnames(HypoMapPred2) = paste(colnames(HypoMapPred2), "_RegSum")

HypoMapRegionPred = predictions$Region_predicted
HypoMapRegionPred2 = HypoMapRegionPred %>% dplyr::select("predicted.id", "prediction.score.max")
HypoMapRegionPred2 = subset(HypoMapRegionPred2, HypoMapRegionPred2$prediction.score.max >= 0.3)
colnames(HypoMapRegionPred2) = paste(colnames(HypoMapRegionPred2), "_RegPred")

MainAssign4 = merge(MainAssign3, HypoMapPred2, by.x = "Row.names", by.y = 0, all =T)
MainAssign5 = merge(MainAssign4, HypoMapRegionPred2, by.x = "Row.names", by.y = 0, all =T)
MainAssign5$`predicted.id _RegSum` = gsub("Arcuate hypothalamic nucleus", "ARC", gsub("Lateral hypothalamic area", "LH", gsub("Ventromedial hypothalamic nucleus", "VMH", gsub("Dorsal premammillary nucleus", "DPN", gsub("Tuberomamillary nucleus", "TM", gsub("Suprachiasmatic nucleus", "SMN", gsub("Paraventricular hypothalamic nucleus", "PVH", gsub("Medial mammillary nucleus", "MMN", gsub("\\(Pre\\)Mammillary region",  "PN", MainAssign5$`predicted.id _RegSum`)))))))))
MainAssign5$`predicted.id _RegPred` = gsub("Arcuate hypothalamic nucleus", "ARC", gsub("Lateral hypothalamic area", "LH", gsub("Ventromedial hypothalamic nucleus", "VMH", gsub("Dorsal premammillary nucleus", "DPN", gsub("Tuberomamillary nucleus", "TM", gsub("Suprachiasmatic nucleus", "SMN", gsub("Paraventricular hypothalamic nucleus", "PVH", gsub("Medial mammillary nucleus", "MMN", gsub("\\(Pre\\)Mammillary region",  "PN", MainAssign5$`predicted.id _RegPred`)))))))))

MainAssign5[is.na(MainAssign5)] = "NA"
#####################

CompilePercentages = as.data.frame(matrix(ncol = 8, nrow = 0))
colnames(CompilePercentages) = c("MRTree_Cluster", "Nuclei", "Nuclei_Alt1", "Nuclei_Alt2", "Nuclei_Alt3", "MRTree_Nuclei", "predicted.id _RegSum", "predicted.id _RegPred")

for(y in unique(MainAssign5$MRTree_Cluster)){
PullClust = subset(MainAssign5, MainAssign5$MRTree_Cluster == y)
Compile = c(y)
for(x in c("Nuclei", "Nuclei_Alt1", "Nuclei_Alt2", "Nuclei_Alt3", "MRTree_Nuclei", "predicted.id _RegSum", "predicted.id _RegPred")){
MainTraject = as.data.frame(table(PullClust[x]))
MainTraject = MainTraject[order(-MainTraject$Freq), ]
MainTraject2 = paste(paste(MainTraject[[1]], " [", round(MainTraject$Freq/dim(PullClust)[1]*100, digits = 1), "%]", sep=""), collapse=", ")
Compile = c(Compile, MainTraject2)
}
Compile = as.data.frame(t(Compile))
colnames(Compile) = c("MRTree_Cluster", "Nuclei", "Nuclei_Alt1", "Nuclei_Alt2", "Nuclei_Alt3", "MRTree_Nuclei", "predicted.id _RegSum", "predicted.id _RegPred")

CompilePercentages = rbind(CompilePercentages, Compile)
}


write.csv(CompilePercentages, "CompilePercentages_AllMethologies.csv", row.names = F)
```
...
MainAssign2 = na.omit(MainAssign2)
...
pdf(paste("Fig2_NP_byNuc_DotPlot_V3.pdf", sep=""), width = 7, height = 20)
print(DotP)
dev.off()
...
NucleiData = EdKaZhouHypoNeurons@assays$SCT@data[c("AGRP", "NPY", "POMC", "CARTPT", "KISS1"), colnames(EdKaZhouHypoNeurons)]
...
#EARLY = subset(NucleiData2, NucleiData2$SampleAdult %in% c("GW10", "GW12", "GW15"))
NucleiData2$`AGRP/NPY` = ifelse(NucleiData2$AGRP > 1.5  & NucleiData2$NPY > 2, 1, 0)
NucleiData2$`POMC/CARTPT` = ifelse(NucleiData2$POMC > 0  & NucleiData2$CARTPT > 3 , 1, 0)
NucleiData2$`POMC/NPY` = ifelse(NucleiData2$POMC > 0  & NucleiData2$NPY > 2, 1, 0)

NucleiData2$NPY = ifelse(NucleiData$NPY > 2  & NucleiData2$`AGRP/NPY` <= 0 & NucleiData2$`POMC/CARTPT` <= 0 & NucleiData2$`POMC/NPY` <= 0, 1, 0)
NucleiData2$POMC = ifelse(NucleiData$POMC > 0  & NucleiData2$`AGRP/NPY` <= 0 & NucleiData2$`POMC/CARTPT` <= 0 & NucleiData2$`POMC/NPY` <= 0, 1, 0)
NucleiData2$AGRP = ifelse(NucleiData$AGRP > 1.5  & NucleiData2$`AGRP/NPY` <= 0 & NucleiData2$`POMC/CARTPT` <= 0 & NucleiData2$`POMC/NPY` <= 0, 1, 0)
NucleiData2$CARTPT = ifelse(NucleiData$CARTPT > 3  & NucleiData2$`AGRP/NPY` <= 0 & NucleiData2$`POMC/CARTPT` <= 0 & NucleiData2$`POMC/NPY` <= 0, 1, 0)
NucleiData2$KISS1 = ifelse(NucleiData$KISS1 > 1  & NucleiData2$`AGRP/NPY` <= 0 & NucleiData2$`POMC/CARTPT` <= 0 & NucleiData2$`POMC/NPY` <= 0, 1, 0)
...
NucleiData5$Sample  = factor(NucleiData5$Sample, levels = c("POMC", "KISS1", "AGRP", "NPY", "CARTPT", "AGRP/NPY", "POMC/NPY", "POMC/CARTPT"))
NucleiData5$Time  = factor(NucleiData5$Time, levels = c("GW10",  "GW12",  "GW15",  "GW16",  "GW18",  "GW19",  "GW20",  "GW22",  "GW25", "Adult"))

CoexpPlot = ggplot(NucleiData5, aes(x= Sample, y = Count, fill = Time)) +geom_col() + theme_classic() +xlab("") + ylab("n positive cells") + theme(axis.text.x = element_text(angle = 90, hjust =1, vjust = 0.5)) +scale_y_continuous(expand = c(0,0))+ scale_fill_manual(values = TimepointColors_FetalAdult)
...
```{r}
##REORGANIZE METADATA
MusRefEdKaZhou = readRDS("~/Downloads/Sci Adv/MusRefEdKaZhouHypoNeurons_mt10_integrated.rds")
...
MissingCells = subset(hypoMap, cells = colnames(MusRefEdKaZhou), invert=T)
Idents(MissingCells) = "C2_named"
hypoMapMissingCellsNeuro = subset(MissingCells, idents = c("C2-1: Neurons"))


CheckMeta = subset(hypoMap@meta.data, hypoMap@meta.data$Dataset %in% c("KimDev10x", "RomanovDev10x"))
CheckMeta$Barcs = gsub("_SRR9303105", "", gsub("_SRR9214103", "", gsub("-.*", "", row.names(CheckMeta))))

CheckMetaMusKa = subset(MusRefEdKaZhou@meta.data, MusRefEdKaZhou@meta.data$StudyAll %in% c("Kim [Mouse Development]", "Romanov [Mouse Development]"))
CheckMetaMusKa$Barcs = gsub(".*_", "", row.names(CheckMetaMusKa))

DefInHypoMap = subset(CheckMetaMusKa, CheckMetaMusKa$Barcs %in% CheckMeta$Barcs)

length(intersect(CheckMeta$Barcs, CheckMetaMusKa$Barcs))

Read = readRDS("~/Downloads/GSE132355_E10-P45_log2cpm.rds") #Counts Matrix? - Blackshaw
ReadSeu = CreateSeuratObject(Read)

Read = readRDS("~/Downloads/GSE132730_TractNAE_integrated.rds") #Romanov Seurat
##
...
Clust10 = subset(POMC.combined.sct_V2_UMAP, row.names(POMC.combined.sct_V2_UMAP) %in% colnames(POMC.combined.Rest4) & POMC.combined.sct_V2_UMAP$UMAP_1 0.5 & POMC.combined.sct_V2_UMAP$UMAP_2 > 0)
Clust4Extra = subset(POMC.combined.sct_V2_UMAP, row.names(POMC.combined.sct_V2_UMAP) %in% colnames(POMC.combined.Rest4) & POMC.combined.sct_V2_UMAP$UMAP_1 0.5 & POMC.combined.sct_V2_UMAP$UMAP_2 < 0)
